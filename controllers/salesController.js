...
const populateInvoiceQuery = (query) => query
  .populate('order', 'orderNumber status totalPrice details.quantity items')
...
exports.createInvoice = async (req, res) => {
  try {
    ...
    const quantity = hasItems
      ? invoiceItems.reduce((sum, i) => sum + i.quantity, 0)
      : (Number(order?.details?.quantity) || 0);
    const unitPrice = hasItems
      ? (totalAmount / quantity)
      : getUnitPriceFromOrder(order);
...
exports.createSalesReturn = async (req, res) => {
  try {
    ...
    const firstProductId = order.items && order.items.length > 0
    ...
    const orderedQuantity = order.items && order.items.length > 0
  ...
  try {
    ...
    const data = invoices.flatMap((inv) => {
      ...
      return lineItems.map((item, idx) => ({
        'Invoice Number': idx === 0 ? inv.invoiceNumber : '',
        'Customer': idx === 0 ? (inv.customer?.name || 'N/A') : '',
        'Product': item.name || inv.product?.name || 'N/A',
        'SKU': item.sku || '',
        'Quantity': item.quantity,
        'Unit Price': item.unitPrice,
        'Subtotal': item.quantity * item.unitPrice,
        'Total Amount': idx === 0 ? inv.totalAmount : '',
        'Paid Amount': idx === 0 ? inv.paidAmount : '',
        'Outstanding': idx === 0 ? Math.max((Number(inv.totalAmount) || 0) - (Number(inv.paidAmount) || 0), 0) : '',
        'Issued Date': idx === 0 ? (inv.issuedDate ? inv.issuedDate.toISOString().split('T')[0] : 'N/A') : '',
        'Due Date': idx === 0 ? (inv.dueDate ? inv.dueDate.toISOString().split('T')[0] : 'N/A') : '',
...
exports.exportSalesOverview = async (req, res) => {
  try {
    ...
    const data = processing.flatMap((item) => {
      const lineItems = getOrderLineItems(item);
      return lineItems.map((line, idx) => ({
        'Order Number': idx === 0 ? item.orderNumber : '',
        'Customer': idx === 0 ? (item.customer?.name || 'N/A') : '',
        'Product': line.product?.name || (item.product?.name || 'N/A'),
        'SKU': line.sku || '',
        'Variant': line.size && line.color ? `${line.size} / ${line.color}` : '',
        'Quantity': line.quantity,
        'Unit Price': line.unitPrice || 0,
        'Line Total': (line.quantity || 0) * (line.unitPrice || 0),
        'Total Amount': idx === 0 ? (item.totalPrice || 0) : '',
        'Payment Total': idx === 0 ? (item.paymentTotal || 0) : '',
        'Outstanding': idx === 0 ? (item.outstandingAmount || 0) : '',
        'Return Qty': idx === 0 ? (item.returnQuantity || 0) : '',
        'Return Amount': idx === 0 ? (item.returnAmount || 0) : '',
        'Status': idx === 0 ? item.commercialStatus : '',
      }));
    });
