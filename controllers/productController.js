...
exports.getPopularProducts = async (req, res) => {
    try {
        ...
        const salesStats = await Order.aggregate([
            ...
            { $limit: 8 } // Ambil lebih banyak untuk di-filter nanti
        ]);
        ...
        // Map data penjualan ke produk
        let popularProducts = products.map(product => {
            const stat = salesStats.find(s => s._id.toString() === product._id.toString());
            const productData = serializeProduct(product);
            productData.totalSold = stat.totalQuantity;
            return productData;
        }).sort((a, b) => b.totalSold - a.totalSold);

        // Jika produk populer kurang dari 4, ambil produk terbaru sebagai tambahan
        if (popularProducts.length < 4) {
            const popularIds = popularProducts.map(p => p._id.toString());
            const fallbackProducts = await Product.find({ 
                _id: { $nin: popularIds } 
            }).sort({ createdAt: -1 }).limit(4 - popularProducts.length);
            
            const fallbackData = fallbackProducts.map(p => {
                const productData = serializeProduct(p);
                productData.totalSold = 0;
                return productData;
            });
            
            popularProducts = [...popularProducts, ...fallbackData];
        }

        res.json(popularProducts.slice(0, 4));
    } catch (error) {
...
// @desc    Ambil produk dengan stok rendah (di bawah threshold masing-masing)
exports.getLowStockProducts = async (req, res) => {
    try {
        ...
        const lowStock = all.filter((p) => {
            ...
            return totalStock < (p.minStockAlert || 100);
        }).sort((a, b) => {
