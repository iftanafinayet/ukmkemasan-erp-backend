...
exports.getAdminStats = async (req, res) => {
  try {
    // 1. Hitung Statistik Utama (pisah Sample vs Regular)
    const totalOrders = await Order.countDocuments();
    const sampleOrders = await Order.countDocuments({ orderType: 'Sample' });
    const regularOrders = totalOrders - sampleOrders;
    const totalCustomers = await User.countDocuments({ role: 'customer' });
    
    // Hitung Total Omzet (Hanya dari order Regular yang sudah lunas)
    const salesData = await Order.aggregate([
      { $match: { isPaid: true, orderType: { $ne: 'Sample' } } },
    ...
    // 3. Produk Terlaris (Top 5, exclude Sample)
