...
exports.updateLandingContent = async (req, res) => {
  try {
    ...
    if (payload.aboutSection) {
      const aboutImageFile = uploadedFiles.get('aboutImage');

      if (aboutImageFile) {
        if (content.aboutSection?.imagePublicId) {
          await deleteFromCloudinary(content.aboutSection.imagePublicId);
        }
        const uploadedImage = await uploadToCloudinary(aboutImageFile.buffer, 'landing-content');
        payload.aboutSection.imageUrl = uploadedImage.url;
        payload.aboutSection.imagePublicId = uploadedImage.publicId;
      } else if (payload.aboutSection.removeImage) {
        if (content.aboutSection?.imagePublicId) {
          await deleteFromCloudinary(content.aboutSection.imagePublicId);
        }
        payload.aboutSection.imageUrl = '';
        payload.aboutSection.imagePublicId = '';
      }

      content.aboutSection = {
        ...(content.aboutSection?.toObject ? content.aboutSection.toObject() : content.aboutSection || {}),
