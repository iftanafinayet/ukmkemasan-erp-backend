...
const loadDescriptionsFromCsv = async () => {
  const descPath = path.join(__dirname, 'data', 'cleaned_product_descriptions.csv');
  if (!fs.existsSync(descPath)) return new Map();

  const fileStream = fs.createReadStream(descPath);
  ...
  for await (const line of rl) {
    ...
    const [name, sku, description] = parseLine(line);
    if (name && description) {
      descMap.set(name, description);
    }
  }
  return descMap;
};
...
const buildProductsFromCsv = async () => {
  const marketingDescriptions = await loadDescriptionsFromCsv();
  ...
  return Array.from(productMap.values()).map((product) => {
    ...
    // Prioritize marketing description from CSV
    let marketingDesc = marketingDescriptions.get(product.name);
    if (marketingDesc) {
      // Remove trailing dot if we are going to join with '. '
      if (marketingDesc.endsWith('.')) {
        marketingDesc = marketingDesc.slice(0, -1);
      }
      descriptionParts.push(marketingDesc);
    }
